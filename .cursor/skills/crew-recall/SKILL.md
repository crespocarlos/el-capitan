...
- **Overview** ("what do you know?", "what's stored?", "journal status") → run `~/.agent/tools/journal-search.py summary`
- **Repo patterns** ("what do you know about kibana?") → run `~/.agent/tools/journal-search.py auto-recall <repo>`
- **Specific question** ("how do retries work?", "what did I learn about worktrees?") → run `~/.agent/tools/journal-search.py query "<text>"`
...
- Always use `~/.agent/tools/journal-search.py` commands — don't read journal files sequentially
