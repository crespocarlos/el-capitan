...
Draft `$TASK_DIR/SPEC.md` using `~/.agent/_SPEC_TEMPLATE.md` as the base:
   ...
   - **Tests**: populate from the explorer's `## Tests` output using this routing: test command → `### Automated > Command` field; test file paths → `### Automated > Scenarios` list; `### Manual` content comes from task context (requirements, edge cases), not from the explorer.
