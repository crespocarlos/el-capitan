...
Draft `$TASK_DIR/SPEC.md` using `~/.agent/_SPEC_TEMPLATE.md` as the base:
   ...
   - **Tests**: populate using two independent judgment rules:
     ...
     2. **Runbook generation**: if any acceptance criterion requires a **live environment** (running server, database, browser, or external service) or **human judgment** (visual check, quality assessment, behavioral check) to verify, generate `$TASK_DIR/runbook.md` alongside the typed blocks. Use the canonical runbook format defined in the References section of the spec (the runbook is surfaced by `crew test` at test time, not auto-executed end-to-end).
